...
import { TaskDetail, TaskStatus, TaskPriority, TaskMember } from "../../types/task";
...
function getInitials(name?: string | null) {
    ...
    export default function TaskDetailModal({
        ...
        // Null guard — before all handlers
        if (!task) return null;

        // ✅ Capture into const so TypeScript holds the non-null narrowing
        // inside startTransition and async callbacks
        ...
        function update<K extends keyof Omit<TaskDetail, "comments">>(key: K, value: TaskDetail[K]) {
            startTransition(() => { onUpdate(currentTask.id, { [key]: value }); });
        }
        ...
        return (
            <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
                <DialogContent className="max-w-2xl max-h-[90vh] overflow-hidden flex flex-col p-0 gap-0">
                    ...
                    <div className="h-1 w-full shrink-0" style={{ backgroundColor: currentTask.projectColor }} />
                    ...
                    <DialogHeader className="px-6 pt-5 pb-0 shrink-0">
                        ...
                        {editingTitle ? (
                            ...
                            <div
                                ...
                                onClick={() => { setTitleDraft(currentTask.title); setEditingTitle(true); }}
                            >
                        ...
                        <div className="flex items-center justify-between mt-2 pb-4 border-b border-border/50">
                            <div className="flex items-center gap-1.5">
                                <div className="w-2.5 h-2.5 rounded-sm" style={{ backgroundColor: currentTask.projectColor }} />
                                <span className="text-xs text-muted-foreground">{currentTask.projectName}</span>
                            </div>
                            <Button
                                variant="ghost"
                                size="sm"
                    ...
                    <div className="flex-1 overflow-y-auto px-6 py-5 space-y-6">
                        ...
                        <div className="grid grid-cols-2 gap-x-4 gap-y-4">
                            ...
                            <div className="space-y-1.5">
                                ...
                                <Select value={currentTask.status} onValueChange={(v) => update("status", v as TaskStatus)}>
                            ...
                            <div className="space-y-1.5">
                                ...
                                <Select value={currentTask.priority} onValueChange={(v) => update("priority", v as TaskPriority)}>
                                    <SelectTrigger className="h-8 text-xs">
                                        <SelectValue />
                                    </SelectTrigger>
                                    <SelectContent>
                        ...
                        <div className="space-y-1.5">
                            ...
                            {editingDesc ? (
                                <div className="space-y-2">
                                    ...
                                    <div className="flex gap-2">
                                        ...
                                        <Button size="sm" variant="ghost" className="h-7 text-xs"
                                            onClick={() => { setDescDraft(currentTask.description ?? ""); setEditingDesc(false); }}>
                                            Cancel
                                        </Button>
                                    </div>
                                </div>
                                ...
                                <div
                                    onClick={() => { setDescDraft(currentTask.description ?? ""); setEditingDesc(true); }}
                                ...
                                >
                                    {currentTask.description
                                        ? <p className="whitespace-pre-wrap text-foreground leading-relaxed">{currentTask.description}</p>
                        ...
                        <div className="text-xs text-muted-foreground flex items-center gap-3 border-t border-border/40 pt-3">
                            ...
                            <span>Updated {formatDate(currentTask.updatedAt)} at {formatTime(currentTask.updatedAt)}</span>
                        </div>
...
export default function TaskDetailModal({
    ...
    // ✅ Null guard BEFORE all handlers — TypeScript now knows task is non-null below
    if (!task) return null;
